...
This chapter describes the development of the artifact, \texttt{ARKIVO.ART}, following the RQDD method introduced in \autoref{chap:method}.
...
The decision of \emph{how} to group the Miles into Iterations really depends on the complexity of each Mile, and their relationships and dependencies.
...
\autoref{fig:dev-plan} illustrates a model of this reverse-planning forward-execution method.
...
Below is a summary of the five Miles. We will write them in the reverse order, as now we are going back towards our goal, see \autoref{tab:rq1-miles}.
...
With these in place we draft an iteration plan. This can always be changed later but will give a rough idea of how many iterations are required, see \autoref{tab:iter-plan}.
...
In this section, the first iteration will be implemented, see \autoref{tab:iter-rq1-miles}.
...
\label{tab:iter-rq1-miles}
\end{table}


\subsection {Mile 5 - Determine search criteria}
...
To achieve this a basic \texttt{ArtifactsController} was created, see \autoref{fig:mile4-arch}.
...
This concludes iteration RQ1.1. At this stage our database contains the metadata of 21,394 OBJKTs.


\section {Iteration RQ1.2}
\label{sec:rq1.2}

In this section iteration RQ1.2 will be developed, which consists of a single Mile, see \autoref{fig:iter-rq1.2}.
...
\label{fig:iter-rq1.2}
\end{table}


\subsection {Mile 3 - Fetch and pin OBJKTs}
\label{sub:rq1_mile3}
...
At this stage the system architecture is illustrated in \autoref{fig:mile3-arch}, and after all OBJKTs are fetched and pinned, the IPFS node is using 79.3GB of disk size.

\label{txt:rq3-answered}

As a by-product of completing Iteration RQ1.2, the research question RQ3, ``what are the disk space requirements for storing all code-based HEN OBJKTs’ media asset'', is already answered.

\section {Iteration RQ1.3}

Iteration RQ1.3 contains 2 Miles, as show in \autoref{tab:iter-rq1.3}:
...
\label{tab:iter-rq1.3}
\end{table}



\subsection {Mile 2 - Run OBJKT in sandbox and capture traffic}
...
\subsection {Mile 1 - Classify Networked OBJKTs}
...
Therefore the plan for RQ2 only contains one Iteration with one Mile, as show in \autoref{tab:iter-rq2}.
...
This RQ only requires one iteration, see \autoref{tab:iter-rq4}.
...
\begin{figure}[h]
    ...
    \setlength{\fboxsep}{0pt} % No padding between box and content
    \setlength{\fboxrule}{1pt} % Thickness of the border
    \fcolorbox{gray}{white}{\includegraphics[width=0.50\linewidth]{arkivo-landing.png}}
    \endgroup
...
With the firewall in place, and docker containers deployed, \texttt{ARKIVO} is live and accessible at: \texttt{arkivo.art}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{hetzner-firewall.png}
...
\begin{lstlisting}[language=, caption={Artifact page metadata}] 
...
One potential solution is to decentralised the IPFS pinning of snapshots by running IPFS nodes at home, see \autoref{fig:offload-snapshots}. Such nodes, whilst not enjoying the same bandwidth of a data centre, would still provide a valuable service by pinning the assets, making them available to other, faster, nodes who can cache them. Doing this only for snapshots would ensure that \texttt{ARKIVO} still performed reasonably well when serving the actual web pages with the artwork assets. Off-loading all snapshots would free up nearly 30GB of storage, which constitutes 20\% of the total server storage. This would also pave the way to a future decentralised and p2p architecture for \texttt{ARKIVO}, see \autoref{fig:arkivo-p2p}.

\begin{figure}[h]
    \centering
    \includesvg[width=0.75\textwidth]{home-node.svg}
...
The assembly of the hardware is relatively straightforward. Some care is required when attaching the heatsink, as the standoffs supplied with the case are very similar but not identical and need to be attached in the right position relative to the Raspberry pi. The following figures illustrate the process:

\begin{figure}[H]
  \centering
  \begin{subfigure}[b]{0.45\textwidth}
    \centering
    \includegraphics[width=\textwidth]{step-01.png}
  ...
  \begin{subfigure}[b]{0.45\textwidth}
    \centering
    \includegraphics[width=\textwidth]{step-02.png}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.45\textwidth}
    \centering
    \includegraphics[width=\textwidth]{step-03.png}
  ...
  \begin{subfigure}[b]{0.45\textwidth}
    \centering
    \includegraphics[width=\textwidth]{step-04.png}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.45\textwidth}
    \centering
    \includegraphics[width=\textwidth]{step-05.png}
  ...
  \begin{subfigure}[b]{0.45\textwidth}
    \centering
    \includegraphics[width=\textwidth]{step-06.png}
    \caption{Install SDD}
...
\begin{figure}[H]
  \vspace*{-\topskip} % Adjusts the space to force the image to the top
  \centering
  \begin{subfigure}[b]{0.45\textwidth}
    \centering
    \includegraphics[width=\textwidth]{step-07.png}
  ...
  \begin{subfigure}[b]{0.45\textwidth}
    \centering
    \includegraphics[width=\textwidth]{step-08.png}
    \caption{Close case and remove stickers}
    \label{fig:step-08.png}
  \end{subfigure}
  \caption{Completing node assembly}
  \label{fig:node-steps-01-02}
\end{figure}
...
The Raspberry SD Card Imager was used to image an SD card (in this case 16GB, but can be 8GB also) with the latest Raspberry Pi OS (64 bit).

The node was then booted with that image, and we used the Raspbery SD Card Imager utility in that installation to image Raspberry Pi OS onto the SDD.

With the operating system now imaged onto the SSD, the node was rebooted, this time removing the SD card (we will no longer be needing it), since now we can boot directly from the SDD.

The DeskPi Pro drivers and services were installed so that they can automatically control the CPU fan based on the temperature sensors built into the Raspberry Pi. The commands used are listed in \autoref{lst:installdeskpi}.

\begin{lstlisting}[language=, caption={Installation of DeskPi drivers}, label={lst:installdeskpi}] 
sudo apt get update
...
Docker was installed, as illustrated in \autoref{lst:installdocker}:

\begin{lstlisting}[language=, caption={Installation of Docker on Node}, label={lst:installdocker}] 
...
After logging out and logging in again to apply the changes, we could then install and run our \texttt{ARKIVO} node snapshot pinning service. It consists of a basic python program that fetches the snapshots of OBJKTs from \texttt{ARKIVO} and pins them to IPFS using a local IPFS node.
...
This starts the download and pinning job. Hitting Ctrl + A exits the screen, which leaves the job working in the background.

We can then monitor the progress of the tool in real time with: \texttt{tail -f successful\_operations.log}.

\begin{lstlisting}[language=, caption={Output of Snapshot Pinning Job}, label={lst:pinningjob}] 
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.45\textwidth}
    \centering
    \includegraphics[width=\textwidth]{twin-nodes.png}
  ...
  \begin{subfigure}[b]{0.45\textwidth}
    \centering
    \includegraphics[width=\textwidth]{power-consumption.png}
...
In this chapter a detailed account of the implementation of the artifact was provided, following the RQDD method introduced in \autoref{chap:method}. This allowed the artifact's developmentg to efficiently answer the research questions. In order to improve the economic sustainability of the artifact, a hardware module was introduced, which allows the pinning of snapshots from home, which allows the releasing of significant amounts of disk space from the cloud-based installation.